...
const RecomendationViaLike =(request,responce)=>{
    ...
    config.query(
        ...
        "NOTES_RETRANCHES AS (SELECT m.idu,n.ids, note-moy AS note FROM NOTER n, MOYENNES m WHERE m.idu IN (SELECT idu FROM NOTER WHERE ids IN (SELECT ids FROM NOTER WHERE idu="+Idu+")) AND m.idu = n.idu AND n.idu!=1),"+ 
        ...
        "    WHERE n.idu!=n2.idu AND n.idu!=1 AND n2.idu="+Idu+" AND n.ids=n2.ids " +
...
const UpdateMotRecher =(request,responce)=>{
    ...
    config.query(
        ...
        "Update chercher set date_rech = now() where idu = "+idu+" and idm ="+idom+" ;"
...
const  PythonRecherche=(request,responce)=>{
   ...
   const process = spawn('python3',['"../app/search_queries/DEMO TFIDF.py"',Chaine], {shell:true})
